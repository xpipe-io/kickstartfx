...
codesign -vvv --entitlements "$1/mac_app/Entitlements.plist" --options=runtime --force --strict --sign "$MACOS_DEVELOPER_ID_APPLICATION_CERTIFICATE_NAME" "$APP_DIR/Contents/MacOS/xpipe" || true
codesign -vvv --entitlements "$1/mac_app/Entitlements.plist" --options=runtime --force --strict --sign "$MACOS_DEVELOPER_ID_APPLICATION_CERTIFICATE_NAME" "$APP_DIR/Contents/MacOS/xpiped"
